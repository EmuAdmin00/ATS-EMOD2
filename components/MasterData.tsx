...
export const MasterData: React.FC<MasterDataProps> = ({ offices, tacs, positions, employees, items, onAddData, onDeleteData }) => {
  ...
  const menuItems: { id: MasterSubView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'Pegawai', label: 'Employees', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1z"/></svg> },
    ...
    <div className="flex flex-col lg:flex-row gap-10">
      <aside className="w-full lg:w-72">
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-5 sticky top-36">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-6 py-4">Repository</p>
            ...
            {menuItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-black transition-all ${
                  activeTab === item.id
                    ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-200 border-none'
                    : 'text-slate-500 hover:bg-slate-50 border-none'
                }`}
              >
      ...
      <div className="flex-1">
        <div className="bg-white rounded-[3rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[650px]">
          <div className="px-12 py-10 bg-slate-50/40 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-8">
            <div>
              <h3 className="text-3xl font-black text-slate-800 tracking-tighter">{activeTab} List</h3>
              <p className="text-[10px] text-slate-400 font-bold mt-1.5 uppercase tracking-widest">Manage master records for asphalt operations</p>
            </div>
            {!isAdding && (
               ...
               <form onSubmit={handleSubmit} className="p-12 space-y-8 animate-in slide-in-from-bottom-4 duration-300">
                  <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-200 space-y-6">
                    <div>
                        ...
                        <input required className="w-full bg-white border border-slate-200 rounded-2xl px-7 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 font-bold" value={formData.name || formData.officeName || ''} onChange={e => setFormData({...formData, [activeTab === 'Office' ? 'officeName' : 'name']: e.target.value})} placeholder="Type here..." />
                    </div>
                    {activeTab === 'Office' && (
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-1 tracking-widest">City Location</label>
                        <input required className="w-full bg-white border border-slate-200 rounded-2xl px-7 py-4 outline-none focus:ring-4 focus:ring-indigo-500/10 font-bold" value={formData.city || ''} onChange={e => setFormData({...formData, city: e.target.value})} placeholder="e.g. Makassar" />
                      </div>
                    )}
                  </div>
                  <div className="flex gap-4">
                     <button type="submit" className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all">Save Record</button>
               ...
               <table className="min-w-full">
                ...
                <tbody className="divide-y divide-slate-50">
                  {currentData().map(item => (
                    <tr key={item.id} className="group hover:bg-slate-50/50">
                      <td className="px-12 py-8">
                        <p className="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition-colors">{item.main}</p>
                        <p className="text-[10px] text-slate-400 font-mono mt-1 uppercase">UID: {item.id}</p>
                      </td>
                      <td className="px-12 py-8">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-tight">{item.sub}</p>
                      </td>
                      <td className="px-12 py-8 text-right">
                        <button 
                          ...
                          className="text-[10px] font-black text-rose-500 hover:text-white hover:bg-rose-500 px-5 py-2.5 rounded-xl transition-all border border-rose-100 hover:border-rose-500 uppercase tracking-widest"
                        >
                          Delete
                        </button>
                      </td>
                  ...
                  {currentData().length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-12 py-20 text-center text-slate-300 font-bold italic">No records found. Click 'Create New' to start.</td>
                    </tr>
                  )}
                </tbody>
              </table>
